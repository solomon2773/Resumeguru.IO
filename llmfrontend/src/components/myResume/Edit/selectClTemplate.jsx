...
import ClTemplatesModal from "../../aiResume/modals/clTemplatesModal";
...
const SelectClTemplate = ({clTemplateData, setClTemplateSelectProgress}) => {
  ...
  const handleCoverLetterDownload = async (fileType) => {
    ...
    if (fileType == 'pdf') {
      ...
      if (pdfBufferResp.ok) {
        ...
        saveAs(pdfBlob, "ResumeGuruIO_"+clTemplateData.firstName+clTemplateData.lastName+"_"+"_v"+(clTemplateData.coverLetterVersion+1)+".pdf");
      } else {
        toast.error("Error generating document. Please try again.")
      }
    } else {
      const generatedDoc = doc.getZip().generate({ type: 'blob' });
      saveAs(generatedDoc, "ResumeGuruIO_"+clTemplateData.firstName+clTemplateData.lastName+"_"+"_v"+(clTemplateData.coverLetterVersion+1)+".docx");
    }
...
export default SelectClTemplate;
