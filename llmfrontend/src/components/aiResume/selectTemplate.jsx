...
const SelectTemplate = ({templateData, resumeDetailData, advanceFeature}) => {
  ...
  useEffect(()=>{
    ...
    mongodbGetResumeTemplate("resume", advanceFeature && advanceFeature.workExperienceParagraphLayout).then((res) => {
      if(res && res.length > 0){
  ...
  const handleResumeDownload = async (fileType) => {
    try{
      let response = "";
      ...
      if (fileType == 'pdf') {
        ...
        if (pdfBufferResp.ok) {
          ...
          saveAs(pdfBlob, "ResumeGuru_"+user.firstName+user.lastName+"_"+encodeURI(resumeDetailData.resumeTemplateName.replace(/\s+/g, ''))+'.pdf');
        } else {
          toast.error("Error generating document. Please try again.")
        }
      } else {
        const generatedDoc = doc.getZip().generate({ type: 'blob' });
        saveAs(generatedDoc, "ResumeGuru_"+user.firstName+user.lastName+"_"+encodeURI(resumeDetailData.resumeTemplateName.replace(/\s+/g, ''))+".docx");
      }
    } catch (error) {
      toast.error("Error generating document. Please try again.")
    }
