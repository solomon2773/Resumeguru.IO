...
import {
    mongodbGetMyDocumentDataByUserIdAndDocId,
    mongodbGetMyDocumentListByUserId
} from "../../../../helpers/mongodb/pages/user/myDocument";
import {toast} from "react-toastify";
import Docxtemplater from "docxtemplater";
import PizZip from "pizzip";
import { saveAs } from 'file-saver';
import Document from "../../../../components/aiResume/coverLetter";
import {useSelector} from "react-redux";


const UserDashboardMyDocument = () =>{
    const user = useSelector((state) => state.user.profile);

    const [myDocuments, setMyDocuments] = useState([]);
    const [loading, setLoading] = useState(false);
    const [isModalOpenDocument, setIsModalOpenDocument] = useState(false);
    const [myDocumentPopupData, setMyDocumentPopupData] = useState(false);
    const [isModalOpenCreateDocument, setIsModalOpenCreateDocument] = useState(false);
    const [myDocumentUrl, setMyDocumentUrl] = useState("");
    useEffect(() => {

        async function getDocuments() {
            setLoading(true);
            if (user && user.userId) {
                mongodbGetMyDocumentListByUserId(user.userId).then((res) => {
                    if (res && res.length > 0) {
                        setMyDocuments(res);
                    }
                    setLoading(false);
    ...
    const viewMyDocuments = (event) => {
        setMyDocumentPopupData(false);

        const documentIUrl = event.currentTarget.getAttribute('data-url');
        const documentName = event.currentTarget.getAttribute('data-name');

        if (documentIUrl){
            setMyDocumentUrl(documentIUrl);
            setMyDocumentPopupData(documentName);
            setIsModalOpenDocument(true);


        } else {
            toast.error("Document not found, please contact us.");
            return;
        }

    }

    const handleMyDocumentDownload = async () => {
        ...
        saveAs(generatedDoc, "coverLetter_"+encodeURI(myDocumentPopupData.userContent.companyName.replace(/\s+/g, ''))+"_"+encodeURI(myDocumentPopupData.userContent.jobTitle.replace(/\s+/g, ''))+".docx");
    ...
    return (
        <UserDashboardCommonLayout
            parent="home"
            title="User Dashboard - My Document - Resume Guru"
            meta_title="User Dashboard - My Document - Resume Guru"
            meta_desc="User Dashboard - My Document - Resume Guru"
            ogType={"website"}
                ...
                <div className="flex flex-col">
                                ...
                                <div className="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-gray-700">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-600 dark:text-gray-400">
                                            Here you can find all of the resume you have uploaded.
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        <div className="inline-flex gap-x-2">
                                            ...
                                            {/*<div*/}
                                            {/*    onClick={() => {setIsModalOpenCreateDocument(true)}}*/}
                    ...
                    <span className="text-xs font-semibold uppercase tracking-wide text-gray-800 dark:text-gray-200">
                      Uploaded Date
                                    ...
                                    {myDocuments.length > 0 ? myDocuments.map((myDocument, coverLetterIndex) => (
                                        ...
                                        key={"coverLetterIndex_"+coverLetterIndex}>
                                            <td className="h-px w-px whitespace-nowrap">
                                                <button type="button" className="block" data-hs-overlay="#hs-ai-invoice-modal" onClick={viewMyDocuments} data-url={myDocument.documentUrl } data-name={myDocument.fileInfo.name }>
                                                <span className="block px-6 py-2">
                                                  <span className="font-mono text-sm text-blue-600 dark:text-blue-500" >{myDocument.fileInfo && myDocument.fileInfo.name}</span>
                                                </span>
                                                </button>
                                            </td>
                                            ...
                                            <td className="h-px w-px whitespace-nowrap">
                                                ...
                                                <span className="block px-6 py-2">
                                                  <span className="text-sm text-gray-600 dark:text-gray-400">{convertToLocalTime(myDocument.createdAt)}</span>
                                                </span>
                                                </button>
                                            </td>
                                            <td className="h-px w-px whitespace-nowrap">
                                                <button
                                                    ...
                                                    onClick={viewMyDocuments}
                                                    data-url={myDocument.documentUrl } data-name={myDocument.fileInfo.name }>
                                ...
                                <div className="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-gray-200 dark:border-gray-700">
                                    <div>
                                        <p className="text-sm text-gray-600 dark:text-gray-400">
                                            <span className="font-semibold text-gray-800 dark:text-gray-200">{myDocuments.length}</span> results
                                        </p>
                                    </div>
                ...
                <div className="relative p-4 w-full max-w-7xl h-full ">
                    ...
                    <div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                            ...
                            <button
                                type="button"
                                onClick={()=>{setIsModalOpenDocument(!isModalOpenDocument)}}
                        ...
                        {myDocumentPopupData && myDocumentUrl ? (
                            <div >
                                ...
                                <div className="gap-4 m-2 ">
                                    ...
                                    <div className="sm:col-span-2">
                                        <label htmlFor="coverLetterAiGenerated"
                                            ...
                                            File Name : {myDocumentPopupData}
                                        </label>

                                        {/*<div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 1000 }}>*/}
                                            {/*<div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', padding: 20, backgroundColor: '#fff' }}>*/}
                                                <iframe src={myDocumentUrl} width="100%" height="600"></iframe>
                    ...
                    <div className="relative p-4 w-full max-w-7xl h-full ">
                        ...
                        <div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                            <div className="flex justify-between items-center pb-4  rounded-t border-b dark:border-gray-600">
                                ...
                                <button
                                    type="button"
                                    onClick={()=>{setIsModalOpenCreateDocument(!isModalOpenCreateDocument)}}
                                    className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="defaultModal">
                                    <svg aria-hidden="true" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd"></path></svg>
                                    <span className="sr-only">Close modal</span>
                                </button>
...
export default UserDashboardMyDocument;
