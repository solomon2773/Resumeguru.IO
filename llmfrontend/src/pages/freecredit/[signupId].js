...
export default function freecreditSignup() {
    const router = useRouter()
    const { loginWithGoogle, loginWithGithub, loginWithLinkedIn, loginWithYahoo, loginWithMicrosoft} = useAuth();
    let signupId = router.query.signupId;

    const [clientIpAddress, clientDetails] = useClientDetails();
    const [logingIn, setLoggedIn] = useState(false);
    const [promoInvoice , setPromoInvoice] = useState(false);
    const dispatch = useDispatch();

    const partnerLogos = {
        "futurecareer": "/images/logos/partners/Future_Career_logo.jpg",
        "aiconnex": "/images/logos/partners/aiconnex_logo.jpg",
        "aiinmckinney": "/images/logos/partners/aiinmckinney_logo.jpeg",
    };
    //console.log(signupId)

    useEffect(()=>{
        // All features are now free - no credit system needed
        if (signupId && (signupId.toLowerCase() === "sameerranjan" || signupId.toLowerCase() === "freetry2025")){
            setPromoInvoice(true);
        } else if (!signupId) {
            signupId = "StartFreeCredit";
        }
    },[signupId])
    ...
    const googleLoginBtn = (e)=>{
        ...
        loginWithGoogle(signupId, promoInvoice).then(async (result)=>{
    ...
    const githubLoginBtn = (e)=>{
        ...
        loginWithGithub(signupId, promoInvoice).then(async (result)=>{
    ...
    const microsoftLoginBtn = (e)=>{
        ...
        loginWithMicrosoft(signupId, promoInvoice).then(async (result)=>{
    ...
    const yahooLoginBtn = (e)=>{
        ...
        loginWithYahoo(signupId, promoInvoice).then(async (result)=>{
    ...
    return (
        ...
        <CommonLayout
            parent="home"
            title="Resume Guru - AI Interview, Sign Up for Free Resume Builder Credit"
            meta_title="Resume Guru - AI Interview, Sign Up for Free Resume Builder Credit"
            ...
            {logingIn ? (
                ...
                <div className="flex min-h-full flex-col justify-center py-12 sm:px-6 lg:px-8">
                    <div className="sm:mx-auto sm:w-full sm:max-w-md">
                        ...
                        <h1 className="mt-2 text-center text-3xl font-bold tracking-tight text-gray-900">Sign up for free credit</h1>
                        ...
                        <div className="bg-white shadow sm:rounded-lg sm:px-10">
                            ...
                            <div className="mt-2">
                                <div className="relative">
                                    ...
                                    {/*</div>*/}
                                    {signupId && signupId.toLowerCase() === "futurecareer" && (
                                        <div>
                                            <img src={process.env.SITE_URL+partnerLogos.futurecareer} alt="Future Career" className="p-3 mx-auto h-40 w-auto" />
                                        </div>
                                    )}
                                    {signupId && signupId.toLowerCase() === "aiconnex" && (
                                        <div>
                                            <img src={process.env.SITE_URL+partnerLogos.aiconnex} alt="AI Connex" className="p-3 mx-auto h-40 w-auto" />
                                        </div>
                                    )}
                                    {signupId && signupId.toLowerCase() === "aiinmckinney" && (
                                        <div>
                                            <img src={process.env.SITE_URL+partnerLogos.aiinmckinney} alt="AI In McKinney" className="p-3 mx-auto h-auto w-auto" />
                                        </div>
                                    )}
                                    <div className="relative flex justify-center text-sm">
                                        <h2 className="bg-white px-2 text-gray-500">Upon registration, you will have free access to all AI-powered features on the site, including resume rewriting, mock interviews, and career tools.</h2>
                                    </div>
                                </div>
